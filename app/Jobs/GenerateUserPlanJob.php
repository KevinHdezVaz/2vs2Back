 ...
 private function calculateFixedMacronutrients($calories, $goal): array
{
    ...
    // Definir porcentajes según objetivo
    ...
    return [
        ...
        'protein' => [
            ...
            'per_kg' => 0 // Agregar campo vacío para compatibilidad
        ],
        'fats' => [
    ...
    {
        // SIEMPRE estructura de 5 comidas según nuevo PDF
        return [
            'structure' => '3 comidas principales + 2 snacks',
            'distribution' => [
                'Desayuno' => 0.25,      // 25%
                'Snack AM' => 0.10,       // 10%
                'Almuerzo' => 0.30,       // 30%
...
private function generateSnackOptions($targetCalories, $isLowBudget, $snackType = 'AM'): array
{
    ...
    // Opciones de proteína para snacks
    if ($isLowBudget) {
        $options['Proteínas'] = [
            'options' => [
                [
                    'name' => 'Yogurt griego',
                    'portion' => sprintf('%dg', round($targetProtein * 10)),
                    'calories' => round($targetProtein * 5),
                    'protein' => $targetProtein,
                    'fats' => round($targetProtein * 0.3),
                    'carbohydrates' => round($targetProtein * 0.5)
                ],
                [
                    'name' => 'Atún en lata',
                    'portion' => sprintf('%dg', round($targetProtein * 3.5)),
                    'calories' => round($targetProtein * 4.5),
                    'protein' => $targetProtein,
                    'fats' => round($targetProtein * 0.1),
                    'carbohydrates' => 0
                ]
            ]
        ];
        ...
        $options['Proteínas'] = [
            'options' => [
                [
                    ...
                    'portion' => sprintf('%dg', round($targetProtein * 1.25)),
                    'calories' => round($targetProtein * 4),
                    'protein' => $targetProtein,
                    'fats' => round($targetProtein * 0.05),
                    'carbohydrates' => round($targetProtein * 0.1)
                ],
                [
                    'name' => 'Yogurt griego alto en proteína',
                    'portion' => sprintf('%dg', round($targetProtein * 7)),
                    'calories' => round($targetProtein * 5.5),
                    'protein' => $targetProtein,
                    'fats' => round($targetProtein * 0.2),
                    'carbohydrates' => round($targetProtein * 0.3)
                ],
                [
                    'name' => 'Caseína',
                    'portion' => sprintf('%dg', round($targetProtein * 1.25)),
    ...
    // Opciones de carbohidratos para snacks (según PDF)
    $options['Carbohidratos'] = [
        'options' => [
            [
                'name' => 'Cereal de maíz',
                'portion' => sprintf('%dg', round($targetCarbs * 1.1)),
                'calories' => round($targetCarbs * 3.8),
                'protein' => round($targetCarbs * 0.07),
                'fats' => round($targetCarbs * 0.03),
                'carbohydrates' => $targetCarbs
            ],
            [
                'name' => 'Crema de arroz',
                'portion' => sprintf('%dg', round($targetCarbs * 1.2)),
                'calories' => round($targetCarbs * 3.6),
                'protein' => round($targetCarbs * 0.06),
                'fats' => round($targetCarbs * 0.01),
                'carbohydrates' => $targetCarbs
            ],
            [
                'name' => 'Galletas de arroz',
                'portion' => sprintf('%d unidades', round($targetCarbs / 7)),
                'calories' => round($targetCarbs * 3.9),
                'protein' => round($targetCarbs * 0.08),
                'fats' => round($targetCarbs * 0.03),
                'carbohydrates' => $targetCarbs
            ],
            [
                ...
                'calories' => round($targetCarbs * 3.75),
                'protein' => round($targetCarbs * 0.13),
                'fats' => round($targetCarbs * 0.07),
                'carbohydrates' => $targetCarbs
            ]
        ]
    ];
    ...
    if ($isLowBudget) {
        $options['Grasas'] = [
            'options' => [
                [
                    'name' => 'Mantequilla de maní casera',
                    'portion' => sprintf('%dg', round($targetFats * 2)),
                    'calories' => round($targetFats * 11),
                    'protein' => round($targetFats * 0.5),
                    'fats' => $targetFats,
        ...
        $options['Grasas'] = [
            'options' => [
                ...
                [
                    'name' => 'Miel',
                    'portion' => sprintf('%dg', round($targetFats * 3)),
                ...
                [
                    ...
                    'portion' => sprintf('%dg', round($targetFats * 1.8)),
